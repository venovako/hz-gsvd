SUBROUTINE SBACKSH(NBSIZE, NBL, NC, IPL, IFCS, INVP, M, N, F, LDF, G, LDG, V, LDV)
  ...
  DO IBL = 1, NBL
     ...
     IF (IPL(IBL) .NE. IBL) THEN
        ...
        IF (IC0 .EQ. IC1) THEN
           ...
           DO I = 1, NCMIN
              CALL SSWAP(M, F(1, IC0 + I), 1, F(1, IC2 + I), 1)
              CALL SSWAP(M, G(1, IC0 + I), 1, G(1, IC2 + I), 1)
              CALL SSWAP(M, V(1, IC0 + I), 1, V(1, IC2 + I), 1)
           END DO
        ...
        ELSE
           ...
           DO I = 1, NCMIN
              CALL SCOPY(M, F(1, IC2 + I), 1, F(1, IC0 + I), 1)
              CALL SCOPY(M, G(1, IC2 + I), 1, G(1, IC0 + I), 1)
              CALL SCOPY(M, V(1, IC2 + I), 1, V(1, IC0 + I), 1)
     ...
     ELSE
        ...
        IF (IC0 .LT. IC1) THEN
           DO I = 1, NC(IBL)
              CALL SCOPY(M, F(1, IC1 + I), 1, F(1, IC0 + I), 1)
              CALL SCOPY(M, G(1, IC1 + I), 1, G(1, IC0 + I), 1)
              CALL SCOPY(M, V(1, IC1 + I), 1, V(1, IC0 + I), 1)
           END DO
        END IF
...
END SUBROUTINE SBACKSH

SUBROUTINE DBACKSH(NBSIZE, NBL, NC, IPL, IFCS, INVP, M, N, F, LDF, G, LDG, V, LDV)
