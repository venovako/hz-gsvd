SUBROUTINE MY_DZIMMER2BO(RANK, FAST, M, N, F, LDF, G, LDG, V, LDV, NBBL, NBMAXS, MAXCYC, TOL,&
  ...
  INTEGER, INTENT(OUT) :: IWORK(*), NBSWP, NBROT(2), INFO(2)
  ...
  DO ITER = 1, MAXCYC(1)
     ...
     IF (FAST) THEN
        IF (MYROT(1) .EQ. 0) GOTO 8
     ELSE
        IF (MYROT(2) .EQ. 0) GOTO 8
     END IF
  END DO
...
SUBROUTINE DZIMMER2BO(P, BP, M, N, F, LDF, G, LDG, V, LDV, MAXCYC, TOL, NBMAXS, H, K, SIGMA, WORK, LWORK, IWORK, LIWORK,&
  ...
  INTEGER, INTENT(OUT) :: IWORK(*), NSWEEP, NROT(2), INFO(2)
  ...
  NEST = OMP_GET_NESTED()
  IF (BP .GT. 1) CALL OMP_SET_NESTED(.TRUE.)

  !$OMP PARALLEL NUM_THREADS(MY_P) DEFAULT(SHARED) PRIVATE(RANK,I,NT) PROC_BIND(SPREAD)
