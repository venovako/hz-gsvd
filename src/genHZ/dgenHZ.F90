program dgenHZ
  ...
  if (idist_f .ne. 0) then
    call dgenlam(n, iseed, idist_f, eps_f, scal_f, ds_f, npos, info)
  else
    call dtxtlam(sigma_f, n, ds_f, npos, info)
  end if
  if (info .ne. 0) then
    write (*,*) info
    stop 'sigma_f'
  ...
  if (idist_g .ne. 0) then
    call dgenlam(n, iseed, idist_g, eps_g, scal_g, ds_g, npos, info)
  else
    call dtxtlam(sigma_g, n, ds_g, npos, info)
  end if
  if (info .ne. 0) then
    write (*,*) info
    stop 'sigma_g'
  ...
  if (idist_x .ne. 0) then
    call dgenlam(n, iseed, idist_x, eps_x, scal_x, dl_x, npos, info)
  else
    call dtxtlam(lambda_x, n, dl_x, npos, info)
  end if
  if (info .ne. 0) then
    write (*,*) info
    stop 'lambda_x'
  ...
  p = 3 * n
  allocate(xwork(p))
  call dgendat(n, iseed, xs_f, xs_g, xl_x, xf, xg, xx, df, dg, du, lda, xwork, info)
  if (info .ne. 0) then
    write (*,*) info
    stop 'dgendat'
  end if
...
contains
  ...
  subroutine readcl(sigma_f, sigma_g, lambda_x, seed, n, fil, grp, &
    ...
    if (command_argument_count() .lt. nrqp) then
      write (*,*) 'dgenHZ.exe SIGMA_F SIGMA_G LAMBDA_X SEEDIX N FILE GROUP [ SIG|LAM_PARAMS ]'
