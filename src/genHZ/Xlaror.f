...
*>          = 1:  if the random numbers generated by XLARND are bad.
*> \endverbatim
*
*  Authors:
*  ========
...
*
*     .. Parameters ..
      REAL(WP)   ZERO, ONE, TOOSML
      PARAMETER          ( ZERO = 0.0E+0_WP, ONE = 1.0E+0_WP,
     $                   TOOSML = 1.0E-20_WP )
*     ..
*     .. Local Scalars ..
...
*     .. External Functions ..
      LOGICAL            LSAME
      REAL(WP)   XLARND, XNRM2
      EXTERNAL           LSAME, XLARND, XNRM2
*     ..
*     .. External Subroutines ..
      EXTERNAL           XGEMV, XGER, XLASET, XSCAL, XERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, SIGN
*     ..
...
*
      IF( LSAME( INIT, 'I' ) )
     $   CALL XLASET( 'Full', M, N, ZERO, ONE, A, LDA )
*
...
*
            CALL XGEMV( 'T', IXFRM, N, ONE, A( KBEG, 1 ), LDA,
     $                  X( KBEG ), 1, ZERO, X( 2*NXFRM+1 ), 1 )
            CALL XGER( IXFRM, N, -FACTOR, X( KBEG ), 1, X( 2*NXFRM+1 ),
     $                 1, A( KBEG, 1 ), LDA )
*
         END IF
*
...
*
            CALL XGEMV( 'N', M, IXFRM, ONE, A( 1, KBEG ), LDA,
     $                  X( KBEG ), 1, ZERO, X( 2*NXFRM+1 ), 1 )
            CALL XGER( M, IXFRM, -FACTOR, X( 2*NXFRM+1 ), 1, X( KBEG ),
     $                 1, A( 1, KBEG ), LDA )
*
         END IF
   30 CONTINUE
*
      X( 2*NXFRM ) = SIGN( ONE, XLARND( 3, ISEED ) )
*
*     Scale the matrix A by D.
*
      IF( ITYPE.EQ.1 .OR. ITYPE.EQ.3 ) THEN
         DO 40 IROW = 1, M
            CALL XSCAL( N, X( NXFRM+IROW ), A( IROW, 1 ), LDA )
   40    CONTINUE
      END IF
*
      IF( ITYPE.EQ.2 .OR. ITYPE.EQ.3 ) THEN
