PROGRAM EXE1
  ...
  CALL READCL(H5FN, H5GN, H5RN, NBMAXS, BP)
  ...
  ISTATS(1) = 0
  IF (NBMAXS .GE. 0) THEN
     CALL DZIMMER1BO(M, N, DSTATS, LDF, DSTATS, LDG, DSTATS, LDV, -1, TOL, NBMAXS, IPART,&
          DSTATS, DSTATS, DSTATS, DSTATS, LWORK, ISTATS, LIWORK, NBSIZE, NBSW, NROT, INFO)
  ELSE
     CALL DZIMMER1NS(M, N, DSTATS, LDF, DSTATS, LDG, DSTATS, LDV, -1, TOL, -NBMAXS, IPART,&
          DSTATS, DSTATS, DSTATS, DSTATS, LWORK, ISTATS, LIWORK, NBSIZE, NBSW, NROT, INFO)
  END IF
  ...
  ELSE
     CALL DZIMMER1NS(M, N, F, LDF, G, LDG, V, LDV, -1, TOL, -NBMAXS, IPART,&
          H, K, SIGMA, WORK, LWORK, IWORK, LIWORK, NBSIZE, NBSW, NROT, INFO)
  END IF
...
CONTAINS
  ...
  SUBROUTINE READCL(H5FN, H5GN, H5RN, NBMAXS, BP)

    IMPLICIT NONE

    CHARACTER(LEN=*), INTENT(OUT) :: H5FN, H5GN, H5RN
    INTEGER, INTENT(OUT) :: NBMAXS, BP

    CHARACTER(LEN=8) :: CLA

    IF (COMMAND_ARGUMENT_COUNT() .GT. 5) STOP 'exe1.exe H5FN H5GN H5RN NBMAXS BTHRDS'
