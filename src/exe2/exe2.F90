PROGRAM EXE2
  ...
  INTEGER :: IDADIM(4)
  INTEGER :: P, M, N, LDA, MAXCYC(2), NBMAXS, LCOLV
  INTEGER :: LWORK, LIWORK, NSWEEP, INFO(2), ISTATS(4)
  INTEGER(8) :: NROT(2), CLK(3)
  DOUBLE PRECISION :: TOL, DSTATS(2)
  !DIR$ ATTRIBUTES ALIGN: 64:: IDADIM, MAXCYC, INFO, ISTATS, NROT, CLK, DSTATS
  ...
  ALLOCATE(H(N)); H = D_ZERO
  ALLOCATE(K(N)); K = D_ZERO
  ALLOCATE(SIGMA(N)); SIGMA = D_ZERO

  ALLOCATE(TAU(N)); TAU = D_ZERO
  DSTATS = D_ZERO
  LWORK = -1
